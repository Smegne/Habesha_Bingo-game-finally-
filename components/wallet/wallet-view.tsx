...
export function WalletView() {
  const { user, deposits, withdrawals, transactions, requestDeposit, requestWithdrawal, convertBonus } =
    useGameStore()
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card className="bg-primary text-primary-foreground">
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-bold">{user?.balance.toFixed(2)} Birr</div>
          </CardContent>
        </Card>
        <Card className="bg-secondary">
          <CardContent className="p-4">
            <div className="text-sm text-secondary-foreground/80">Bonus Balance</div>
            <div className="text-2xl font-bold text-secondary-foreground">
              {user?.bonusBalance.toFixed(2)} Birr
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <Dialog open={showDepositDialog} onOpenChange={setShowDepositDialog}>
          <DialogTrigger asChild>
            <Button className="flex-col h-auto py-3">
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={depositMethod}
                  onValueChange={(v) => setDepositMethod(v as "telebirr" | "cbe")}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="p-3 bg-muted rounded-lg">
                ...
                <p className="text-lg font-mono">0911-123-456</p>
        ...
        <Dialog open={showWithdrawDialog} onOpenChange={setShowWithdrawDialog}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Withdraw your winnings to your mobile wallet</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div className="space-y-2">
                <Label>Amount (Birr)</Label>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setWithdrawMethod(v as "telebirr" | "cbe")}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
          ...
          <Card>
            <CardContent className="p-0">
              {userTransactions.length === 0 ? (
                <p className="text-muted-foreground text-sm p-4 text-center">No transactions yet</p>
              ) : (
                <div className="divide-y max-h-64 overflow-y-auto">
                  {userTransactions.slice().reverse().map((tx: any) => (
                    <div key={tx.id} className="flex items-center justify-between p-3">
                      ...
                      <span
                        className={`font-semibold ${tx.amount >= 0 ? "text-primary" : "text-destructive"}`}
                      >
                        {tx.amount >= 0 ? "+" : ""}
                        {tx.amount} Birr
                      </span>
          ...
          <Card>
            <CardContent className="p-0">
              {userDeposits.length === 0 ? (
                <p className="text-muted-foreground text-sm p-4 text-center">No deposits yet</p>
              ) : (
                <div className="divide-y max-h-64 overflow-y-auto">
                  {userDeposits.slice().reverse().map((deposit: any) => (
                    <div key={deposit.id} className="flex items-center justify-between p-3">
                      <div>
                        <div className="text-sm font-medium">{deposit.amount} Birr</div>
                        <div className="text-xs text-muted-foreground">
                          via {deposit.method} - {new Date(deposit.createdAt).toLocaleString()}
                        </div>
                      </div>
                      <Badge
                        variant={
          ...
          <Card>
            <CardContent className="p-0">
              {userWithdrawals.length === 0 ? (
                <p className="text-muted-foreground text-sm p-4 text-center">No withdrawals yet</p>
              ) : (
                <div className="divide-y max-h-64 overflow-y-auto">
                  {userWithdrawals.slice().reverse().map((withdrawal: any) => (
                    <div key={withdrawal.id} className="flex items-center justify-between p-3">
                      <div>
                        <div className="text-sm font-medium">{withdrawal.amount} Birr</div>
                        <div className="text-xs text-muted-foreground">
                          to {withdrawal.accountNumber} - {new Date(withdrawal.createdAt).toLocaleString()}
                        </div>
                      </div>
                      <Badge
                        variant={
